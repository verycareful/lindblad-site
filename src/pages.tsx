    ...
    <section style={{ paddingTop: 80, paddingBottom: 64, borderTop: 0 }}>
      <div className="container">
        <div className="flex gap-12 items-center">
          ...
          <span className="eyebrow">RELEASE 1.1.27.2 · 3076/3077 TESTS PASSING · 271 SUITES</span>
        </div>
        ...
        <div className="grid mt-64" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
          ...
          <Stat label="LINES OF C++" value={<Counter to={42000} suffix="+" />} sub="3077 unit tests · 271 test suites" />
        </div>
      </div>
    </section>
  );
...
`<span class="fn">FetchContent_Declare</span>(
    ...
    <span class="cm"># GIT_TAG        1.1.27.2</span>
)
...
<span class="fn">target_link_libraries</span>(my_app <span class="ty">PRIVATE</span> lindblad_core)`
            ...
            <div className="flex justify-between items-center" style={{ padding: '10px 18px', borderTop: '1px solid var(--rule)', background: 'var(--surface-2)' }}>
              <span className="mono text-faint" style={{ fontSize: 11 }}>LATEST RELEASE · 1.1.27.2</span>
...
$ ctest --test-dir build --output-on-failure
   3076/3077 tests passed across 271 suites`
              }} />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-12 mt-16" style={{ flexWrap: 'wrap' }}>
          ...
          <a href="https://github.com/verycareful/lindblad/releases/tag/1.1.27.2" target="_blank" rel="noopener" className="btn">↗ 1.1.27.2 RELEASE NOTES</a>
        </div>
      </div>
    </section>
  );
...
// ---------------------------------------------------------------------------
// Benchmarks page data — transcribed from docs/Benchmarks.md, the page
// GENERATED by tools/bench_report.py. Update BENCH_RUN + BENCH_DOMAINS +
// BENCH_PARITY when the report is regenerated. Cadence policy: benchmark runs
// are refreshed periodically, NOT for every release — BENCH_RUN.version is the
// version stamped by the run that produced these numbers and may lag the
// release version shown in the site chrome.
// ---------------------------------------------------------------------------
...
const BENCH_RUN = {
  ...
  parity: '7/7 PASS',
  paired: '49/49 workloads paired',
};

type BenchRow = { w: string; lb: number; aer: number; x: number; lbq?: string; aerq?: string };

const BENCH_PARITY: { w: string; detail: string }[] = [
  { w: 'val__clifford__ladder__n8', detail: 'TVD 0.0000 (support 256, noise scale 0.0997)' },
  { w: 'val__dm__scaling__n6', detail: 'TVD 0.0070 (support 42, noise scale 0.0404)' },
  { w: 'val__sv__grover__s8', detail: 'TVD 0.0002 (support 4, noise scale 0.0125)' },
  { w: 'val__sv__qft__n8', detail: 'TVD 0.0984 (support 256, noise scale 0.0997)' },
  { w: 'val__sv__qv__n8', detail: 'TVD 0.0875 (support 252, noise scale 0.0990)' },
  { w: 'val__sv__scaling__n8', detail: 'TVD 0.0934 (support 256, noise scale 0.0997)' },
  { w: 'val__est__heisenberg__n8', detail: '|lindblad − aer| = 3.4e-13' },
];

const BENCH_DOMAINS: { name: string; note: string; quality?: boolean; rows: BenchRow[] }[] = [
  {
    name: 'Statevector',
...
function BenchTable({ domain }: { domain: (typeof BENCH_DOMAINS)[number] }) {
  return (
      ...
      <table className="mono" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ borderBottom: '1px solid var(--rule-strong)' }}>
            <th style={{ textAlign: 'left', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>WORKLOAD</th>
            <th style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>LINDBLAD (MS)</th>
            <th style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>AER (MS)</th>
            <th style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>SPEEDUP</th>
            {domain.quality ? (
              <>
                <th style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>LB 2Q/DEPTH</th>
                <th style={{ textAlign: 'right', padding: '12px 16px', fontWeight: 400, color: 'var(--ink-faint)' }}>AER 2Q/DEPTH</th>
              </>
            ) : null}
          </tr>
        </thead>
        <tbody>
          {domain.rows.map((r) => (
            <tr key={r.w} style={{ borderBottom: '1px solid var(--rule)' }}>
              <td style={{ padding: '10px 16px' }} className="text-dim">{r.w}</td>
              <td style={{ padding: '10px 16px', textAlign: 'right' }}>{r.lb.toFixed(3)}</td>
              <td style={{ padding: '10px 16px', textAlign: 'right' }}>{r.aer.toFixed(3)}</td>
              <td style={{ padding: '10px 16px', textAlign: 'right', color: r.x >= 1 ? 'var(--green)' : 'var(--orange)' }}>
                {r.x.toFixed(2)}x
              </td>
              {domain.quality ? (
                <>
                  <td style={{ padding: '10px 16px', textAlign: 'right' }} className="text-dim">{r.lbq}</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right' }} className="text-dim">{r.aerq}</td>
                </>
              ) : null}
            </tr>
          ))}
  ...
  return (
    <PageFrame active="benchmarks">
      <section style={{ paddingTop: 64, paddingBottom: 48, borderTop: 0 }}>
        <div className="container">
          ...
          <h1 className="h-display mt-24" style={{ fontSize: 'clamp(48px, 6vw, 96px)' }}>
            Measured against
            <br />
            <span style={{ color: 'var(--orange)' }}>the incumbent.</span>
          </h1>
          <p className="lead mt-32">
            Head-to-head against Qiskit / Qiskit Aer on a shared, committed QASM2 corpus. Both engines run natively under a
            mirrored protocol (warmup, 5 repetitions, median), out-of-box on both sides. No timing table is published unless
            both engines provably compute the same answers.
          </p>
          <div className="card mt-32" style={{ padding: '16px 22px' }}>
            <div className="flex justify-between items-center" style={{ flexWrap: 'wrap', gap: 12 }}>
              <span className="mono" style={{ fontSize: 13 }}>
                <span className="text-faint">BENCHMARK RUN · </span>
                <span style={{ color: 'var(--orange)' }}>{BENCH_RUN.version}</span>
                <span className="text-faint"> · {BENCH_RUN.date} · {BENCH_RUN.paired} · parity {BENCH_RUN.parity}</span>
              </span>
              <span className="text-dim" style={{ fontSize: 12, maxWidth: 560 }}>
                Benchmark runs are refreshed periodically, not for every release: the version shown is stamped by the run
                that produced these numbers and may lag the current release.
              </span>
            </div>
          </div>
        </div>
      ...
      <section>
        <div className="container">
          <div className="grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
            <Stat label="MPS DOMAIN" value="10–23x" sub="faster across n=16–40, every bond dimension" accent="var(--green)" />
            <Stat label="PEAK STATEVECTOR" value="72x" sub="QFT n=10 · small/mid circuits typically 4–72x" accent="var(--green)" />
            <Stat label="EXACT ESTIMATOR" value="38x" sub="Heisenberg n=20, shots=0 vs dense expectation" accent="var(--green)" />
            <Stat label="RESULT PARITY" value="7/7" sub="TVD within sampling noise; expectations to 3e-13" accent="var(--blue)" />
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="container">
          <SectionHeader
            kicker="RESULT PARITY"
            title="Correctness gate first"
            desc="Every published number is gated on cross-engine result parity at 8192 shots: total-variation distance on sampled counts against a sampling-noise-aware threshold, and exact agreement (1e-6) on estimator expectation values. The Grover entry peaks on a non-symmetric marked state, so the gate doubles as an end-to-end qubit-ordering convention check between the engines."
          />
          <div className="card" style={{ padding: '18px 22px' }}>
            {BENCH_PARITY.map((p) => (
              <div key={p.w} className="flex justify-between mono" style={{ fontSize: 13, padding: '6px 0', flexWrap: 'wrap', gap: 8 }}>
                <span className="text-dim">{p.w}</span>
                <span>
      ...
      {BENCH_DOMAINS.map((d, i) => (
        <section key={d.name}>
          <div className="container">
            <SectionHeader kicker={`DOMAIN ${String(i + 1).padStart(2, '0')}`} title={d.name} desc={d.note} />
            <BenchTable domain={d} />
          </div>
        </section>
      ))}
      ...
      <section>
        <div className="container">
          <SectionHeader
            kicker="READ IT STRAIGHT"
            title="Where each engine wins"
            desc="speedup = aer_ms / lindblad_ms; above 1.00x means Lindblad is faster. Both columns below come from the same gated run."
          />
          ...
          <p className="text-dim mt-32" style={{ fontSize: 13 }}>
            Full methodology, environment capture, and the generated report live in <span className="mono text-blue">docs/Benchmarks.md</span>;
            the corpus, both harnesses, and the parity validator ship in-tree (<span className="mono text-blue">benchmarks/</span>) so every
            number here is reproducible with the command sequence in <span className="mono text-blue">docs/BuildAndTest.md</span>.
          </p>
        </div>
      </section>
    </PageFrame>
...
const FAQ_ITEMS = [
  ...
  { q: 'Can I publish benchmarks comparing LINDBLAD to other simulators?', a: 'Yes, under the academic license. We ask that you cite the release version you benchmarked (e.g. 1.1.27.2) and report hardware specs alongside any performance numbers.' },
...
const KEY_TERMS = [
  ...
  { n: '§14', title: 'Third-party components', body: 'This software incorporates open-source dependencies (Eigen, NLopt, GoogleTest, Google Benchmark, pybind11). Their license notices are in the NOTICE file. Your obligations under those licenses are independent of and in addition to this Agreement.' },
] as const;
...
<span class="ty">version</span>:      <span class="st">"1.1.27.2"</span>
<span class="ty">date-released</span>: <span class="st">"2026-09-04"</span>
...
const THIRD_PARTY_DEPS = [
  { name: 'Eigen', version: '5.0.0', license: 'MPL-2.0', note: 'Primarily MPL-2.0; individual files carry Apache-2.0 or BSD-3-Clause terms, reproduced in NOTICE. Contains no GPL or LGPL code.' },
  { name: 'NLopt', version: '2.7.1', license: 'LGPL-2.1-or-later', note: 'Statically linked. Lindblad is source-available, so you can recompile against a modified NLopt (satisfies LGPL §6 relinking).' },
  { name: 'GoogleTest', version: '1.14.0', license: 'BSD 3-Clause', note: 'Test suite only — not linked into any production binary.' },
  { name: 'Google Benchmark', version: '1.8.3', license: 'Apache 2.0', note: 'Optional benchmark suite only — not linked into any production binary.' },
  { name: 'pybind11', version: '2.12.0', license: 'BSD 3-Clause', note: 'Optional Python bindings (LINDBLAD_BUILD_PYTHON=ON).' },
] as const;
...
function ThirdPartyNotices() {
  return (
    <section>
      <div className="container">
        <SectionHeader
          kicker="THIRD-PARTY COMPONENTS · NOTICE"
          title="Open-source dependencies."
          desc="These components are fetched at build time via CMake FetchContent. Their license notices are reproduced in full in the NOTICE file in the repository."
        />
        <div className="card" style={{ padding: 0 }}>
          {THIRD_PARTY_DEPS.map((dep, index) => (
            <div key={dep.name} className="flex" style={{ borderBottom: index === THIRD_PARTY_DEPS.length - 1 ? 0 : '1px solid var(--rule)', padding: '20px 28px', gap: 32, alignItems: 'flex-start' }}>
              <div style={{ minWidth: 160, flex: '0 0 160px' }}>
                <div className="mono" style={{ fontSize: 14, fontWeight: 500 }}>{dep.name}</div>
                <div className="mono text-dim" style={{ fontSize: 12, marginTop: 2 }}>{dep.version}</div>
              </div>
              <div style={{ minWidth: 140, flex: '0 0 140px' }}>
                <span className="mono text-orange" style={{ fontSize: 13 }}>{dep.license}</span>
              </div>
              <div className="text-dim" style={{ fontSize: 13, lineHeight: 1.6, flex: 1 }}>{dep.note}</div>
            </div>
          ))}
        </div>
      </div>
